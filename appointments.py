...
def get_locations(location_group):
    ...
    headers = {"Connection": "close", "User-Agent": "TwitterVaccineBot", "Content-Type": "application/json;charset=UTF-8", "Origin": "https://myturn.ca.gov", "Sec-Fetch-Site": "same-site", "Sec-Fetch-Mode": "cors", "Sec-Fetch-Dest": "empty", "Referer": "https://myturn.ca.gov/", "Accept-Encoding": "gzip, deflate", "Accept-Language": "en-US,en;q=0.9"}

    for lat, lng in GEOCODES[location_group]:
        today = str(date.today())
        payload={"doseNumber": 1, "fromDate": today, "location": {"lat": lat, "lng": lng}, "locationQuery": {"includePools": ["Alameda", "Alameda_Contra_Costa", "Alpine", "Amador", "Butte", "Calaveras", "Colusa", "Contra_Costa", "Del_Norte", "El_Dorado", "Fresno", "Glenn", "Humboldt", "Imperial", "Inyo", "Kern", "Kings", "Lake", "Lassen", "Long_Beach", "Los_Angeles", "Madera", "Marin", "Mariposa", "Mendocino", "Merced", "Modoc", "Mono", "Monterey", "Napa", "Nevada", "Orange", "Pasadena", "Placer", "Plumas", "Riverside", "Sacramento", "San_Benito", "San_Bernardino", "San_Diego", "San_Francisco", "San_Joaquin", "San_Luis_Obispo", "San_Mateo", "Santa_Barbara", "Santa_Clara", "Santa_Cruz", "Shasta", "Sierra", "Siskiyou", "Solano", "Sonoma", "Stanislaus", "Sutter", "Tehama", "Trinity", "Tulare", "Tuolumne", "Ventura", "Yolo", "Yuba", "default"]}, "url": "https://myturn.ca.gov/location-select", "vaccineData": "WyJhM3F0MDAwMDAwMEN5SkJBQTAiLCJhM3F0MDAwMDAwMDFBZExBQVUiLCJhM3F0MDAwMDAwMDFBZE1BQVUiLCJhM3F0MDAwMDAwMDFBZ1VBQVUiLCJhM3F0MDAwMDAwMDFBZ1ZBQVUiLCJhM3F0MDAwMDAwMDFBc2FBQUUiXQ=="}
...
def get_appt_days(id, vaccine_data, start, end, dose=1):
    # Generated by burp
    url = f"https://api.myturn.ca.gov:443/public/locations/{id}/availability"
    headers = {"Connection": "close", "User-Agent": "TwitterVaccineBot", "Content-Type": "application/json;charset=UTF-8", "Origin": "https://myturn.ca.gov", "Sec-Fetch-Site": "same-site", "Sec-Fetch-Mode": "cors", "Sec-Fetch-Dest": "empty", "Referer": "https://myturn.ca.gov/", "Accept-Encoding": "gzip, deflate", "Accept-Language": "en-US,en;q=0.9"}
    payload={"doseNumber": dose, "endDate": str(end), "startDate": str(start), "url": "https://myturn.ca.gov/appointment-select", "vaccineData": vaccine_data}

    resp = make_proxied_request('POST', url, headers=headers, json=payload)
    if not resp:
        return []
...
def get_slots(id, vaccine_data, day):
    url = f"https://api.myturn.ca.gov:443/public/locations/{id}/date/{day}/slots"
    headers = {"Connection": "close", "User-Agent": "TwitterVaccineBot", "Content-Type": "application/json;charset=UTF-8", "Origin": "https://myturn.ca.gov", "Sec-Fetch-Site": "same-site", "Sec-Fetch-Mode": "cors", "Sec-Fetch-Dest": "empty", "Referer": "https://myturn.ca.gov/", "Accept-Encoding": "gzip, deflate", "Accept-Language": "en-US,en;q=0.9"}
    payload={"url": "https://myturn.ca.gov/appointment-select", "vaccineData": vaccine_data}
    resp = make_proxied_request('POST', url, headers=headers, json=payload)
    if not resp:
        return []
...
def main():
    ...
    parser.add_argument('--initial-delay',
        help='Amount of time in seconds to delay before first batch',
        type=int,
        default=0,
        required=False,
    )
    ...
    if args.initial_delay:
        print(f'Sleep {args.initial_delay} seconds before starting up')
